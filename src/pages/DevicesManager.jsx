...
function DevicesManager() {
    ...
    const HandleAddDevice = () => {
        if (!lat && !lon) {
            ...
            axios.post('http://localhost:5000/api/newDevice', { newDeviceName, newDeviceSerial, lat, lon, username })
                .then((response) => {
    ...
    const HandleDelDevice = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`http://localhost:5000/api/deleteDevice/${id}`)
                    .then((response) => {
                        setAddMessage(response.data);
                        Swal.fire({
                            title: "Deleted!",
    ...
    const HandleUpdateDevice = () =>{
        axios.put(`http://localhost:5000/api/updateDevice/${id}`, { newDeviceName, newDeviceSerial, lat, lon, username })
                .then((response) => {
                    setAddMessage(response.data);
                    setShow(false);
                    Swal.fire({
